...
**Linux development test host:** from the local workstation, `ssh -o IdentitiesOnly=yes remote-session-runner` reaches `ubuntu` on `oracle-yuta-konopka-ubuntu-micro-02` (Ubuntu 20.04.6 LTS). Read-only preflight on 2026-09-25 found no Podman package candidate, hybrid/cgroup-v1 controls, and insufficient evidence of the required rootless limits/capacity. This host is **not approved** as the `linux-sandbox` baseline; replace or deliberately rebuild and validate it **before `P001`** under §1. The SSH alias is for diagnosis until then, not a PASS for Linux/two-host gates. Record the identity of the eventual validated host in each host-test evidence file. Do not use this ordinary development host for destructive physical power-cut tests unless it is explicitly made disposable and approved for that purpose.<br>
