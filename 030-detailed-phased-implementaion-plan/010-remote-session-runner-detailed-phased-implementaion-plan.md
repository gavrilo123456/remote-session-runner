...
**Target:** one macOS workstation using `tomasz.walczuk` for local services/commands and one Linux host using `ubuntu` for host services/commands; a shared Go execution core with target-specific process adapters. On both targets commands inherit their respective user's OS permissions. This PoC has no Podman/container runtime or Runner filesystem confinement.<br>
**Linux development test host:** from the local workstation, `ssh -o IdentitiesOnly=yes remote-session-runner` reaches `ubuntu` on `oracle-yuta-konopka-ubuntu-micro-02` (Ubuntu 20.04.6 LTS). The absence of Podman and cgroup-v2 controls found on 2026-09-25 does not block this host-process PoC. The planned direct HTTPS endpoint is the host's public `129.151.232.40:8443` with mandatory mTLS, not a tunnel or private-network route. The SSH observation is connectivity evidence only, not a PASS for Linux process, source, service, public HTTPS, or two-host gates. Validate the `linux-host` process profile on this named machine at its real-host phases, and record the machine identity in each result. If a required host-process or connectivity gate fails, stop under §1.2; do not silently switch hosts or relax the gate. Do not use this ordinary development host for destructive physical power-cut tests unless it is explicitly made disposable and approved for that purpose.<br>
...
Before `P001`, create `040-implementation-evidence/000-preimplementation-decisions.md` and record the selected Mac and Linux accounts, owner-only service paths, the Linux development test host above, the SSH key/host-key/controller mapping, and the public direct-HTTPS endpoint `129.151.232.40:8443`, its actual host-local listen address/port, mTLS server/client identities, CA trust, IP SAN, and controller-principal mapping. Record the intended firewall/source restrictions if the host/provider can enforce them; they supplement and never replace mandatory mTLS. Define each actual environment, not just its name: allowed target-kind/profile pairs, local base system, an explicit allow/deny policy for each source mode (`empty`, `git_revision`, `local_worktree`) including approved repository aliases and the local-worktree access rule where applicable, any requested mounts/volumes (disabled for this PoC), effective Runner-enforced service ceilings, and controller identities authorized to create and control it. Both local and Linux workspace paths are start directories, not filesystem confinement; commands have the selected account's OS permissions. The `mac-dev`/`linux-dev` environment names do not imply isolation. Review and commit this prerequisite note in a **separate, non-phase commit**, then start `P001` from a clean worktree and record that prerequisite commit hash in its preflight. Use these committed decisions as input to `P006` validation and `P010` registry/configuration; neither phase may fill gaps by guessing. These are pre-implementation choices, not proof that deployment already works; the later named host gates must still pass. No Podman/rootless/cgroup feasibility check is required before `P001`. If a required choice is unresolved, resolve it or amend the design and plan before implementation starts.
...
| `P006` | Add environment/target/source/limit validation and truthful capability types, separating Runner service ceilings from unavailable host isolation controls. | `D-02` pure validation subset; incompatible or unenforceable policy rejects, and neither process profile advertises sandbox confinement. |
...
The selected Mac account and designated Linux host/profile must exist before their real-host gates. A fake adapter cannot certify either. Use the same persistent-shell contract cases on both targets. The Linux `linux-host` adapter launches Bash/children as `ubuntu`; it does not install Podman, create a container, or claim per-session CPU, memory, PID, disk, network, mount, or filesystem isolation. Keep the Runner-enforced service ceilings and reject requests requiring unavailable controls.
...
| `P039` | Add Linux `linux-host` process-profile preflight/doctor for the named `ubuntu` host. | Real-host UID, Bash, writable owner-only service paths, process inspection/signaling, and truthful capabilities; no Podman/cgroup check or false isolation claim. |
| `P040` | Add Linux host-process prepare/start/inspect/cleanup with private session workspace and session/generation ownership records. | Real Linux process launch/teardown subset of `P-LNX-01`; Bash and child UID is `ubuntu`, workspace is not a confinement boundary. |
| `P041` | Track Linux agent/Bash process groups and descendants for command stop, teardown, and residual-capacity accounting. | Real-host process/child interruption tests; uncertain stop retains a slot and never launches a replacement shell. |
| `P042` | Reconcile Linux process ownership across `runnerd` restart/generation change; quarantine unknown or surviving descendants. | Real-host kill/restart/orphan subset of `P-LNX-01` and `F-01`; unresolved processes block safe capacity reuse. |
| `P043` | Reject Linux requests for unavailable per-session CPU/memory/PID/disk/network/mount restrictions, and verify actual OS access, status capabilities, and truthful process-teardown outcomes. | Full real-host `P-LNX-01`; confirmed descendants are stopped where possible, while unconfirmed residuals produce `lost`/degraded health and retain capacity. Runner service ceilings still apply; no sandbox or volume capability is advertised. |
| `P044` | Run shared persistent-shell and unsafe-boundary suite on Mac and Linux host-process adapters. | Full real-host `R-01` and `R-02`; unsafe shell loses session without replacement. |
| `P045` | Add Linux exact-Git-revision source preparation using an approved public/local repository alias; support an optional host-side credential reference without injecting it into the command environment/workspace. | Real Linux `P-LNX-02`; moving branch pinned and hook policy verified, with credential fixture only if configured. Since preparation and commands use `ubuntu`, do not claim an OS credential or host-write barrier between them. |
...
### Stage 5 — public direct HTTPS and common CLI (detailed design §§3–4, 10, 14)

Provision server/client certificates, CA trust, controller mapping, and intended direct client before `P106`. Direct HTTPS is reachable at public `129.151.232.40:8443`; the service binds the actual host-local interface/port selected in the kickoff note, which may differ from the public address if the provider uses NAT. Before enabling that listener, validate and record the host/provider firewall and source-address policy, including an explicit rationale if source allowlisting is unavailable; this policy is defense in depth, never a substitute for mTLS. Verify the server certificate's IP SAN and client certificate before any request reaches the authority, with no bearer or unauthenticated fallback. No tunnel or private-network route is required. The direct remote API rejects `local`; the CLI selects ingress explicitly, never from an opaque ID.

| Phase | Bounded deliverable | Focused exit gate |
| --- | --- | --- |
| `P106` | Add in-process HTTPS listener on the selected host-local bind, reachable at public `129.151.232.40:8443`, with mandatory mTLS verification and certificate-principal map; validate and record the selected firewall/source policy before public startup. | Real two-host public TLS/identity subset of `P-NET-02`; verify correct IP SAN and documented ingress policy, and missing/invalid client certificate fails before mutation with no HTTP/bearer fallback. |
...
| `P117` | Add real two-host public-endpoint server-name/IP-SAN checks and controller denial across read/events/mutations. | Full `P-NET-02` against `129.151.232.40:8443`, including valid-client success; no-client, wrong/expired certificate, forged-controller, and cross-controller denials; record firewall/source policy evidence without treating it as auth. |
...
| `P126` | Add Linux `systemd` definitions for `runnerd` as `ubuntu` and forced-command deployment notes. | Real Linux start/stop/restart; invalid host-process profile or owner-only service path not ready. |
...
| `P132` | Wire signal-driven graceful shutdown into runnerd and Linux systemd stop path. | Real Linux host-process test: no false terminal outcome, unaccounted descendant, or unflushed event/audit. |
...
| `P146` | Rerun actual Linux host-process, exact-Git, service, policy-rejection, and cleanup/recovery suites after final changes. | `P-LNX-*`, Linux `R-*`, truthful-capability and process-teardown gates current. |
| `P147` | Rerun two-host SSH, public-endpoint mTLS, reconnect, file-only mailbox, and CLI suites. | `P-NET-*`, `P-MBX-01`, `P-CLI-01` current on named hosts, including public positive/negative TLS cases. |
