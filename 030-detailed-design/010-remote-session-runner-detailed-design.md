...
| Field | Contract |
| --- | --- |
| `session_id`, `command_id` | Opaque, deployment-unique, caller-visible IDs. The Mac allocates queued IDs before dispatch and sends the same IDs to the target executor; direct requests receive or supply stable IDs through the remote API. IDs are never reused within metadata retention. |
...
The endpoint on `exec`, `events`, and `close` must match the creating ingress; an opaque ID never makes the CLI switch endpoint automatically. Status shows target, source, host class, and effective isolation/limits.

| Operation | Proposed route | Local Unix socket | Direct HTTPS |
| --- | --- | --- | --- |
| Create session | `POST /v1/sessions` | `local` or queued `remote`; returns local intent ID and acceptance scope | `remote` only; returns authoritative ID |
...
| `exec_commands` | `command_id` PK; `session_id` FK, contiguous authoritative `ordinal`, optional source Mac `intent_ordinal`, request hash, script reference, state, timeout, exit code, final event sequence, truncation/completeness, timestamps; `UNIQUE(session_id, ordinal)`. | Target executor only. |
...
| `local_intents` | `intent_id` PK; operation, resource ID/session ID, immutable target, canonical hash, idempotency key, `intent_ordinal` for commands, acceptance/delivery state, lease owner/expiry, attempt count, timestamps; `UNIQUE(session_id, intent_ordinal)` for command intents. | Mac API writes intent and allocates per-session command order; Router owns lease/delivery columns. Local executor consumes but does not rewrite request content. |
...
| `remote_session_views`, `remote_command_views` | Remote IDs, last authoritative snapshot, observed time/stale flag; command terminal/output summary. No local execution transition API. | Mac Router from remote reads. |
...
1. **Local ingress:** validate request shape/owner; insert or find `local_intents` and `local_idempotency` in one transaction, plus a requested lifecycle record for session creation. For command submission, allocate `intent_ordinal` transactionally under that session's Mac sequence and keep it immutable. A queued-remote intent starts `delivery_state: recorded`. Replying at this point proves only Mac receipt.
2. **Authoritative session acceptance:** check controller/environment/target compatibility and idempotency; insert `exec_sessions(state=creating)` and its lifecycle record in one transaction. A duplicate with the same hash returns the existing resource; changed hash conflicts. Runtime creation happens *after* this commit.
3. **Authoritative command acceptance:** lock/check session state and allocate the authority's next contiguous `ordinal` for either ingress. For queued work, retain the Mac `intent_ordinal` as provenance, **not** as the authoritative ordinal: an earlier Mac intent may have been definitely cancelled before delivery, leaving an intentional gap in Mac intent numbers. Insert `exec_commands(state=queued)`, `exec_command_events(sequence=1,type=command_queued)`, and idempotency record atomically. Only after commit may the scheduler see it.
4. **Execution start:** under scheduler ownership, atomically assert session `ready`, command is lowest eligible ordinal and `queued`, and host quota is available; set session `busy`, command `running`, and append `command_started` before asking the agent to source the script. A crash after this commit but before actual source is conservatively recovered as `lost`, not retried under the same shell.
...
1. Validate principal/controller, immutable target, environment authorization and target/profile compatibility, source mode, requested limits, and host quota. A local profile rejects a remote-only isolation/limit requirement it cannot enforce; it never advertises sandbox confinement.
2. Persist authoritative `creating` and lifecycle record. Queue mode has already persisted a Mac `requested` intent; direct mode begins here.
...
`run`/`POST /v1/jobs` creates an ephemeral session with the explicit target/environment/source, submits exactly one command, follows its outcome, and requests close. Its result carries both session and command IDs, actual command exit/state/output flags, and teardown state. A successful command with failed teardown is not mislabeled a wholly successful job. It uses the same idempotency and event store as ordinary sessions, not a separate one-shot executor.
...
The agent owns one Bash process and three logically separate paths: an agent-to-shell command channel, a shell-to-agent control channel, and stdout/stderr byte streams. Control frames contain command ID, completion code, and agent generation; they have strict framing and size limits. Scripts are never interpolated into control frames. The agent attributes output only to the active command, reads stdout and stderr concurrently, and sends at most 16 KiB raw chunks (or after about 50 ms under normal load) to the executor. `ExecutionService` persists ordered events before publishing them to subscribers; the agent has no database. Reserved descriptors, agent-prefixed shell variables/functions, and the command channel are outside the user-command contract. The agent detects a broken control channel or unexpected Bash exit and reports loss rather than guessing completion from printed text.

At a command boundary, known child processes must be gone before the shell is reused. The supervisor attempts process-tree cleanup; if a descendant retains a pipe, escapes tracking, or leaves the boundary uncertain, it closes or loses the session. In particular, arbitrary background services are unsupported. Local process-tree controls are best-effort under the account's OS permissions; the design does not claim hostile-code containment on macOS.
...
On restart, expired leases are reclaimed. Local session/command intent remains pending until the appropriate executor returns a known result. A Mac sleep/network outage does not stop a remotely accepted command; the Router reconnects, reads authoritative session/command state, then replays events after its cursor. A remote generation change never implies a usable old shell. The Router mirrors `lost`/terminal outcomes and rejects or cancels work queued behind a terminal session; it never creates a replacement shell under the same session ID.
...
On projector restart, validate each existing NDJSON file through its last complete line/sequence. An incomplete trailing line is truncated; the projector then rebuilds or appends the contiguous prefix from durable SQLite events before advertising any cursor. A terminal response is republished only after its stored frozen cursor is again backed by a synced event file. A reader that holds an old file handle after atomic rebuild reopens the named event file when polling; it does not assume a file handle is the durable identity.
...
| Runtime cleanup fails | Session is `lost`, not `closed`/`expired`; audit and health/reporting expose the residual process/sandbox risk. |
...
| --- | --- |
| Active limits | 20 sessions and four running commands per execution host; one running command/session. |
...
| Metadata/idempotency | At least 90 days on both authorities, including mailbox request mappings; no automatic remote retry after the guarantee expires. |
| Event payloads | 30 days on both authorities, then metadata reports `retention_expired`. Active command/session events are not expired. |
...
| `I-03` | Disconnect an ordinary direct HTTPS event reader after acceptance, reconnect from cursor, disconnect a slow subscriber, and request expired range. | Command continues detached by default; ordered replay does not rerun it or block on slow reader; expired/gapped range yields `event_history_unavailable`, not a successful partial stream. |
| `I-04` | Run one-off job with command success and forced teardown failure. | Response retains command success/exit/output plus failed/lost teardown, not a wholly successful job. |
...
| `M-07` | Two responses reference one command event file; advance fake clock through ACK/no-ACK deadlines. | One ACK never deletes shared file; removal waits for terminal command and every referencing response deadline; status can regenerate retained events. |
...
| `R-02` | Real Bash suite: nonzero exit, stdout/stderr interleave, `exit`, `exec`, reserved-FD sabotage, and background child. | Normal nonzero leaves safe session ready; unsafe boundary loses/closes session; no replacement shell or later queued execution; output stays in correct command/stream. |
...
| `P-NET-01` | Two-host SSH fixture: wrong/changed host key, forced-command escape/PTY/forward attempts. | Host verification fails closed; unauthorized SSH behavior denied; bridge stderr contains no user output. |
...
| `P-CLI-01` | CLI smoke suite across local, queued-remote, and direct-remote on configured hosts. | Same `session create`/`exec`/`events`/`close` verbs work with explicit endpoint; status displays target/isolation; wrong endpoint never inferred; nonzero exit and transport failure render distinctly. |
...
| --- | --- |
| Shared CLI/API behavior, one-off result, persistent shell | `I-01`, `I-04`, `R-01`, `R-02`, `P-CLI-01`, `P-MAC-01`, `P-LNX-01` on the actual target hosts. |
| Immutable target, environment compatibility, no fallback | `D-01`, `D-02`, `D-10`, `I-01`. |
| Authority, durable acceptance, idempotency, ordering | `D-03` through `D-08`, `M-08`, `F-01`, `F-02`. |
| File-only correlation, complete answer, ACK, retention | `M-01` through `M-09`, `O-01` through `O-03`, and two-host `P-MBX-01`. |
...
| Limits, migrations/config, operations, backup, performance | `D-14`, `O-02`, `F-03` through `F-05`, `P-OPS-01`. |
...
| 1. Shared authority core | SQLite store, idempotent acceptance, scheduler, event replay, fake runtime. | `D-03`–`D-09`, `D-13`, `I-01` against fake adapters. |
...
| 4. Mac ingress and mailbox | Unix API, Router, remote mirror/gap, file requests/responses/events/ACKs, one-off job. | `M-01`–`M-09`, `O-01`–`O-03`, `D-09`–`D-10`, `I-04`, then `P-MBX-01`. |
