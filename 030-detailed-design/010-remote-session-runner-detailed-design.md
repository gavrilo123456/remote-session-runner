...
| `exec_jobs` | `job_id` PK; unique controller/one-off key/hash, stable `session_id` and `command_id`, durable phase, last confirmed command and teardown outcomes, timestamps. | Target executor; one-off coordinator checkpoints each step and resumes by these IDs after restart. |
...
3. **Authoritative command acceptance:** lock/check session state and allocate the authority's next contiguous `ordinal` for either ingress. For queued work, retain the Mac `intent_ordinal` as provenance, **not** as the authoritative ordinal: an earlier Mac intent may have been definitely cancelled before delivery, leaving an intentional gap in Mac intent numbers. Insert `exec_commands(state=queued)`, `exec_command_events(sequence=1,type=command_queued)`, and idempotency record atomically. Only after commit may the scheduler see it.
   **One-off acceptance:** first insert/find `exec_jobs` and its idempotency record with stable job/session/command IDs in one transaction; Mac ingress has already committed those IDs with its `run` intent. The job coordinator uses stable derived step keys for session creation, command submission, and close. It checkpoints each phase after the underlying authoritative transaction and, after a crash, reads the IDs/resources before resuming a missing step. It never submits a second command or creates a replacement shell because a checkpoint lagged.
...
| `I-03` | Disconnect an ordinary direct HTTPS event reader after acceptance, reconnect from cursor, commit an event exactly between replay query and live delivery, overflow a slow subscriber during replay, and request expired range. | Command continues detached by default; register-before-replay plus sequence de-duplication never misses or duplicates a handoff event; overflow returns resumable cursor without blocking command; expired/gapped range yields `event_history_unavailable`, not a successful partial stream. |
