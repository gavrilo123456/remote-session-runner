...
| `D-17` | Fill four host command slots, request cancel/timeout on one while a fake runtime delays stop, and race a fifth start; advance retention beyond 90 days with an unresolved residual child. | Fifth command cannot start while cancellation/timeout/residual execution holds a slot; only confirmed stop releases it; live slot and parent command metadata survive GC. |
